   ...
   // Token management
    ...
    SearchDataCoin() {
        $('#tokenSearch').on('keyup', () => {
            ...
            rows.each(function () {
                const row = $(this);
                const textMatches = row.text().toLowerCase().includes(keyword);

                // Cek juga isi input dan select (symbol, pairSymbol, contractAddress, dll)
                ...
                // Tampilkan baris jika cocok di text atau input
                const match = textMatches || inputMatches;
                row.toggle(match);
            });
        });
    }
    ...
    loadTokenTable() {
        ...
        const sortedTokens = this.tokens.slice().sort((a, b) => (b.isActive === true) - (a.isActive === true));
        ...
        sortedTokens.forEach((token, index) => {
            const cexBadges = token.selectedCexs.map(cex => {
                ...
                return `
                    <span class="badge ${this.getBadgeColor(cex, 'cex')} exchange-badge">${cexUpper}:</span>
                    <span class="badge fs-8 biru">${statusText}</span>
                `;
            }).join('<br/>');

            const allDexs = this.getDexList();
            const dexCheckboxes = allDexs.map(dex => {
                const isChecked = token.selectedDexs.includes(dex);
                const checkedAttr = isChecked ? 'checked' : '';
                const colorClass = this.getBadgeColor(dex, 'dex');
                return `<label class="me-2 badge ${colorClass}" style="cursor: pointer;">
                            <input type="checkbox" class="update-dex-checkbox"
                                data-token-id="${token.id}" data-dex="${dex}" value="${dex}" ${checkedAttr}/> ${dex}
                        </label>`;
            }).join('');

            const allCexs = this.getCexList();
            const cexCheckboxes = allCexs.map(cex => {
            ...
            const card = $(`
                <div class="col-12 col-sm-6 col-md-6 col-lg-2">
                    ...
                    <div class="card border shadow-sm h-100 rounded-top no-rounded-bottom">
                        ...
                        <!-- HEADER: putih -->
                        <div class="card-header px-2 py-1 d-flex justify-content-between align-items-center border-bottom-0 biru" style="min-height: unset;">
