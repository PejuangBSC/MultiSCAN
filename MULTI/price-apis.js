...
const DEXAPIs = {
    ...
    // Magpie API
    getMagpiePrice: function(fromToken, toToken, amount, chainName, userAddr) {
        const url = `https://api.fly.trade/aggregator/quote?network=${chainName}&fromTokenAddress=${fromToken}&toTokenAddress=${toToken}&fromAddress=${userAddr}&toAddress=${userAddr}&sellAmount=${amount}&slippage=0.3&gasless=false`;
    ...
    getLiFiPrice: function(fromToken, toToken, amountIn, fromDecimals, toDecimals, chainId, token, direction) {
        ...
        if (direction === 'cex_to_dex') {
            ...
            headers['x-lifi-api-key'] = '36e8ccb2-d4e1-4321-8ca7-d78bae3d1461.5eacae7f-1e16-4ea9-9d5d-f9dcb03f7d50';
        }
        ...
        const requestData = {
            ...
            options: {
                ...
                exchanges: {
                    deny: 
                    [ "jupiter", "sushiswap",  "enso",  "bebop", "dodo",  "stable", "aftermath", "superswap", "bluefin7k", "liquidswap" ]
                    // [
                    //     "1inch",
                    //     "kyberswap",
                    //     "jupiter",
        ...
        return new Promise((resolve, reject) => {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (!routes.length) {
                        return reject({ exchange: 'LiFi', error: 'No route found' });
                    }

                    const results = routes.map((route, idx) => {
                        const tool = route.steps?.[0]?.toolDetails?.name || route.steps?.[0]?.tool || 'Unknown';
                        const rawToAmount = parseFloat(route.toAmount || "0");
                        const priceUSD = parseFloat(route.toAmountUSD || "0");
                        const feeUSD = parseFloat(route.gasCostUSD || 0);

                        const normalizedOut = rawToAmount / Math.pow(10, toDecimals);
                        const normalizedIn = parseFloat(amountIn) / Math.pow(10, fromDecimals);
                        const rate = normalizedOut > 0 ? normalizedOut / normalizedIn : 0;
                        const quotePriceUSDT = priceUSD / normalizedOut;

                        return {
                            exchange: tool,
                            amountOut: rawToAmount,
                            amountIn: amountIn,
                            price: rate,
                            rawRate: rate,
                            quotePriceUSDT,
                  ...
                  //  console.log(`${direction} : ${token.symbol} → ${token.pairSymbol}`);
                    ...
                    //     No: i + 1,
                    //     Tool: r.exchange,
                    //     'Qty Out': r.normalizedOut.toFixed(6),
                    //     'USD Out': `$${r.hasilUSDT.toFixed(4)}`,
                    //     'Fee': `$${r.fee.toFixed(4)}`,
                    //     'Rate': r.price.toFixed(6),
                    //     'Quote/USDT': r.quotePriceUSDT.toFixed(4)
                    // })));

                    resolve(results); // ← array of dexInfo
                },
                ...
                error: function(xhr, status, error) {
                    ...
                    try {
                        const res = JSON.parse(xhr.responseText);
                        errText = res.message || error || status;
                    } catch {
    ...
    getPLUSPrice: function ({ account, chainId, sellToken, sellDecimals, buyToken, buyDecimals, sellAmount, direction, slippage = 0.3, integratorId = 'dzap' }) {
        return new Promise((resolve, reject) => {
            $.ajax({
                ...
                success: function (response) {
                    try {
                        ...
                        if (!result || !result.quoteRates) {
                            return reject({
                                exchange: 'PLUS',
                                error: 'No quoteRates returned',
                        ...
                        for (const provider in quoteRates) {
                            ...
                            const rawOut = parseFloat(quote.destAmount);
                            const rawIn = parseFloat(quote.srcAmount);

                            if (!rawIn || !rawOut || isNaN(rawIn) || isNaN(rawOut)) continue;

                            const price = rawOut / rawIn;
                            const rawRate = rawIn / rawOut;
                            const feeUSD = parseFloat(quote?.fee?.totalUSD || 0);
...
async function checkAllCEXWalletsPerChain() {
    ...
    try {
        ...
        // localStorage.setItem("MULTI_ACTIONS", waktuUpdate);
